...
import './AsminPage.css' // Stile importieren
...
function Admin() {
    ...
    const fetchFirmen = async () => {
        try {
            const response = await fetch('http://localhost:8080/api/firma', {
                method: 'GET',
                headers: {
    ...
    const handleStatusChange = async (firmaId, status) => {
        const response = await fetch(`http://localhost:8080/api/firma/${firmaId}`, {
            method: 'PATCH',
    ...
    const handleDeleteFirma = async (firmaId) => {
        ...
        if (confirmDelete) {
            const response = await fetch(`http://localhost:8080/api/firma/${firmaId}`, { method: 'DELETE' });
            if (response.ok) {
